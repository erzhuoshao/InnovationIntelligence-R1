...
class Index_Builder:
    ...
    def encode_all(self):
        ...
        for start_idx in tqdm(range(0, len(self.corpus), self.batch_size), desc='Inference Embeddings:'):
            ...
            if "specter" in self.retrieval_method:
                # For SPECTER models, use title + sep_token + abstract format
                batch_data = list(self.corpus[start_idx:start_idx+self.batch_size]['content']) # title + [SEP] + abstract

            elif self.retrieval_method == "e5":
            ...
            inputs = self.tokenizer(
                        ...
                        return_token_type_ids="specter" not in self.retrieval_method,
            ).to('cuda')
            ...
            if "T5" in type(self.encoder).__name__:
                ...
                if self.retrieval_method == "specter":
    ...
    def build_dense_index(self):
        ...
        if self.distance_metric == "ip":
            ...
            raise ValueError(f"Unsupported distance metric: {self.distance_metric}. Use 'ip' or 'l2'.")
...
def main():
    ...
    parser.add_argument('--faiss_type', default='Flat', type=str, 
                        help='FAISS index type. Examples: "Flat", "IVF100,Flat", "IVF100,PQ16", "HNSW32"')
    parser.add_argument('--embedding_path', default=None, type=str)
    parser.add_argument('--save_embedding', action='store_true', default=False)
    parser.add_argument('--faiss_gpu', default=False, action='store_true')
    parser.add_argument('--adapter_name', type=str, default=None, help='Adapter name for models like SPECTER2')
    parser.add_argument('--distance_metric', type=str, default='ip', choices=['ip', 'l2'], 
                        help='Distance metric for FAISS index: inner product (ip) or L2 distance (l2)')
